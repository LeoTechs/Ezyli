...
std::pair<std::vector<std::string>,std::vector<std::string>> run(){
    ...
    {
        can_travel_with_r.push_back(result[i]["pick"][EVENT_TRIP_ID_INDEX]);
    }
    ...
    // call exclude_event_ids with L
    exclude = exclude_event_ids(L);
    // save in exclude 
    return std::make_pair(can_travel_with_r,exclude);
...
bool travelCompare_fn(std::map<std::string, std::vector<std::string>> R, std::map<std::string, std::vector<std::string>> C)
{
    ...
    if (C["pick"][EVENT_TYPE_INDEX] == EVENT_TYPE_PICKUP && R["pick"][EVENT_TYPE_INDEX] == EVENT_TYPE_PICKUP && c_and_r_has_same_departure_time && c_and_r_has_same_departure_city && c_and_r_has_same_arrival_city)
    {
        return true;
    }
}
...
int main(void)
{
    ...
    // call run
    std::pair<std::vector<std::string>,std::vector<std::string>> result = checker.run();
    std::vector<std::string> can_travel_with_r = result.first;
    std::vector<std::string> exclude = result.second;
    // print result
